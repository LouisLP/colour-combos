// PROTOTYPE — throwaway. Minimal colour maths, no dependency.
 ...
 * chroma held, until `hex` clears `target` against `against`. Returns the first
...
export function clampToContrast(
  ...
  target: number,
  direction: -1 | 1,
): { hex: string, ratio: number, shifted: number, reached: boolean } {
  const start = col.l
